...
document.addEventListener('DOMContentLoaded', async () => {
    ...
    async function updateControlsForViewingDate() {
        ...
        if (isViewingToday) {
            ...
            if (workdayState === 'INCOMPLETE') {
                ...
                const day = currentlyViewingDate.getDay(); // 0 = Domingo, 6 = Sábado

                // Só perguntar se for dia útil e não houver registros
                if (currentlyViewingDate < new Date() && entries.length === 0 && day !== 0 && day !== 6) {

                    const foiFeriado = confirm(
                        `Nenhuma marcação encontrada no dia ${currentlyViewingDate.toLocaleDateString('pt-BR')}. Esse dia foi feriado?`
                    );
                    ...
                    if (foiFeriado) {
                        ...
                        await supabase.from('holidays').insert({
                            user_id: user.id,
                            date: currentlyViewingDate.toISOString().split('T')[0],
                            description: 'Feriado informado manualmente'
                        });

                        clockButton.textContent = 'Feriado';
                        workdayStatus.textContent = 'Este dia foi marcado como feriado.';
                        return;
                    }
                }

                // Se final de semana, não mostrar como falta
                if (day === 0 || day === 6) {
                    clockButton.textContent = 'Final de Semana';
                    workdayStatus.textContent = 'Dia não útil — final de semana.';
                    return;
                }

                // Padrão: dia útil sem marcação e não feriado
