...
document.addEventListener('DOMContentLoaded', () => {
    ...
    let allMonthTickets = []; // <-- NOVO: Armazena todos os tickets do mês
    ...
    async function processAndRenderData(reportData) {
        // 👇 ETAPA 1: BUSCAR O USUÁRIO E SUA JORNADA DE TRABALHO
        const { data: { user } } = await supabase.auth.getUser();
        // Se o usuário não tiver uma jornada salva, usamos 8 como padrão
        const contractualWorkday = user?.user_metadata?.contractual_workday_hours || 8;

        const { entries, tickets } = reportData;
        allMonthTickets = tickets; // Armazena os tickets para uso posterior
        ...
        // 👇 ETAPA 2: AQUI ESTÁ A MUDANÇA PRINCIPAL NO CÁLCULO
        // Substituímos o '8' pela variável 'contractualWorkday'
    ...
    // <-- NOVO: Função para mostrar os tickets de um dia específico
    function showTicketsForDay(dayIndex) {
        ...
        dailyTicketsTitle.textContent = `Tickets do Dia ${targetDate.toLocaleDateString()}`;
    ...
    function renderDailySummaryChart(dailyWorkSeconds, dailyTicketSeconds) {
        ...
        // <-- GRÁFICOS MAIS BONITOS: Criando gradientes
        ...
        dailySummaryChart = new Chart(ctx, {
                ...
                datasets: [
                    {
                        ...
                        data: workHoursData,
                        backgroundColor: blueGradient, // Gradiente aplicado
                    ...
                    },
                    {
                        ...
                        backgroundColor: emeraldGradient, // Gradiente aplicado
            ...
            options: {
                ...
                // <-- NOVO: Adiciona interatividade no clique
                onClick: (event, elements) => {
                    if (elements.length > 0) {
                        const dayIndex = elements[0].index;
                        showTicketsForDay(dayIndex);
                    }
                },
                ...
                scales: {
                    y: {
                        ...
                        grid: { color: 'rgba(200, 200, 200, 0.1)' } // Grid mais sutil
                    },
                    x: {
    ...
    // Plugin do Chart.js (sem alterações)
                    ...
                    tooltip: {
                        callbacks: {
                            label: function (context) {
                                ...
                                label += formatSecondsToHHMM(valueSeconds);
                                return label;
                            }
                        }
                    },
    ...
    async function initialize() {
        ...
        // Esconde o card de tickets diários ao carregar novos dados
    ...
    // <-- NOVO: Evento para fechar o card de tickets do dia
    closeDailyTicketsBtn.addEventListener('click', () => {
        dailyTicketsCard.classList.add('hidden');
    });
